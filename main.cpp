...
int main(int argc, char *argv[])
{
                 ...
                 << QDBusConnection::sessionBus().lastError().message();
    {
        ...
        // Commented since https://github.com/linuxdeepin/treeland/commit/b9b3a885c4a2ce22ac5171e1c69e9a6730a255e1
        // sleep(0.1);
    ...
    qInfo() << "Basic test Success. Now we start an inhibit and sleep for 100 seconds, and you can use test_idle_inhibit_v1 to test if idle inhibit is exactly working or not.";

    QDBusReply<uint> reply = interface.call("Inhibit", "treeland-screensaver-test", "Testing inhibit/uninhibit");
    uint cookie;
    if (reply.isValid()) {
        cookie = reply.value();
        qInfo() << "Inhibited. Cookie:" << cookie;
    } else {
        qFatal() << "Failed to call Inhibit:" << reply.error().message();
    }
    sleep(100); // Sleep for a while to allow manual testing of idle inhibit
    return 0;
}
